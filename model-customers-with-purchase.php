...
function selectCustomersPurchase($custId) { 
    try { 
        $conn = get_db_connection(); 
        $stmt = $conn->prepare(" 
            SELECT s.sale_id, c.cust_id, c.cust_firstname, c.cust_lastname, p.product_name, s.sale_date, s.tax, s.shipping, si.quantity, si.saleprice 
            FROM Sale s 
...
function UpdateCustomerWithPurchase($cust_id, $cust_firstname, $cust_lastname, $cust_address, $cust_phone, $cust_email) { 
    try { 
        ...
        // Update Sale table (optional, assuming you want to update it too)
